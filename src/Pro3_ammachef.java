...
public class Pro3_ammachef {
	...
	private static Polynomial readPolynomial() throws Exception {
		...
		boolean fileExists = polyFile.exists();
		...
		if (!fileExists) {
			...
			if (line.isEmpty()){
				...
				Polynomial polynomialFromFile = new Polynomial(line);	
				fin.close();
				return polynomialFromFile;
			}
		...
		//throw new Exception("TODO: YOU NEED TO FILL IN THIS METHOD");
	}
	...
	public static void getParamsUser(Minimizer m) {
		...
		System.out.println();
		double eps   = Pro3_ammachef.getDouble("Enter tolerance epsilon [0.000001,infinity]: ", 0.000001, Double.MAX_VALUE);
		m.setEps(eps);

		int max_iter = Pro3_ammachef.getInteger("Enter maximum number of iterations [1,1000]: ", 1, 1000);
		m.setMaxIter(max_iter);

		double alpha = Pro3_ammachef.getDouble("Enter step size alpha [0.000001,1]: ", 0.000001, 1.0);		
		m.setStepSize(alpha);
		...
		while (true) {
			...
			double val = Pro3_ammachef.getDouble("Enter starting value for variable " + var + ": ", -Double.MAX_VALUE, Double.MAX_VALUE);	
			x0.put(var, val);
		}
		m.setX0(x0);
